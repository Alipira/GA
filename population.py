...
class Population:
    ...
    def generate_population(self):
        ...
        while i < 4 * self.params.ap.mu and (i == 0 or self.params.ap.timeLimit == 0 or
                                             (time.process_time() - self.params.startTime) / CLOCKS_PER_SEC <
                                             self.params.ap.timeLimit):
    ...
    def add_individual(self, indiv, update_feasible):
        ...
        if indiv.eval.is_feasible and indiv.eval.penalized_cost < self.best_solution_restart.eval.penalized_cost - MY_EPSILON:
            ...
            if indiv.eval.penalized_cost < self.best_solution_overall.eval.penalized_cost - MY_EPSILON:
                ...
                self.search_progress.append((time.process_time() - self.params.startTime,
                                             self.best_solution_overall.eval.penalized_cost))
            return True
        else:
            return False

    def update_biased_fitnesses(self, pop):
        ranking = sorted(enumerate(pop), key=lambda x: -self.average_broken_pairs_distance_closest(x[1],
                                                                                                    self.params.ap.nbClose))
    ...
    def remove_worst_biased_fitness(self, pop):
        ...
        for i, ind in enumerate(pop[1:], 1):
            ...
            if (is_clone and not is_worst_individual_clone) or (
                    is_clone == is_worst_individual_clone and ind.based_fitness > worst_individual_biased_fitness):
    ...
    def manage_penalties(self):
        ...
        if fraction_feasible_duration < self.params.ap.target_feasible - 0.05 and self.params.penalty_duration < 100000.:
            self.params.penalty_duration = min(self.params.penalty_duration * self.params.ap.penalty_increase,
                                               100000.)
        ...
        for ind in self.infeasible_subpop:
            ind.eval.penalized_cost = ind.eval.distance + self.params.penalty_capacity * ind.eval.capacity_excess + \
                                      self.params.penalty_duration * ind.eval.duration_excess
    ...
    def get_binary_tournament(self):
        ...
        indiv1 = self.infeasible_subpop[place1 - len(self.feasible_subpop)] if place1 >= len(
            self.feasible_subpop) else self.feasible_subpop[place1]
        indiv2 = self.infeasible_subpop[place2 - len(self.feasible_subpop)] if place2 >= len(
            self.feasible_subpop) else self.feasible_subpop[place2]
    ...
    def print_state(self, nb_iter, nb_iter_no_improvement):
        if self.params.verbose:
            print(f"It {nb_iter:6d} {nb_iter_no_improvement:6d} | T(s) {time.process_time() - self.params.start_time:.2f}", end="")

            best_feasible = self.get_best_feasible()
            if best_feasible:
                print(
                    f" | Feas {len(self.feasible_subpop)} {best_feasible.eval.penalized_cost:.2f} {self.get_average_cost(self.feasible_subpop):.2f}", end="")
            else:
            ...
            if best_infeasible:
                print(
                    f" | Inf {len(self.infeasible_subpop)} {best_infeasible.eval.penalized_cost:.2f} {self.get_average_cost(self.infeasible_subpop):.2f}", end="")
            else:
                print(" | NO-INFEASIBLE", end="")

            print(f" | Div {self.getDiversity(self.feasible_subpop):.2f} {self.get_diversity(self.infeasible_subpop):.2f}", end='')
            print(f" | Feas {Counter(self.list_feasibility_load)[True] / len(self.list_feasibility_load):.2f} {Counter(self.list_feasibility_duration)[True] / len(self.list_feasibility_duration):.2f}", end='')
    ...
    def export_search_progress(self, file_name, instance_name):
        with open(file_name, 'w') as file:
            for state in self.search_progress:
                file.write(
                    f"{instance_name};{self.params.ap.seed};{state[1]};{state[0] / CLOCKS_PER_SEC}\n")
